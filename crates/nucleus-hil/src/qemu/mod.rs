...
impl Backend for QemuBackend {
    ...
    fn start(
        ...
        let itm_log_path =
            std::env::temp_dir().join(format!("nucleus-hil-qemu-itm-{}.bin", std::process::id()));
    ...
    /// Polls the semihosting log file for the first instrumentation packet.
    /// M5 only needs to prove one log event is observable; re-polling after
    /// the first event currently re-reads the same bytes from the start of
    /// the file rather than tracking a read cursor — fine for the single-log
    /// fixture this milestone targets, worth revisiting once M6/M7 need
    /// multiple distinct ITM events per run.
    fn await_itm_event(&mut self, timeout: Duration) -> Result<Option<ItmEvent>, HilError> {
        ...
        let event = runtime.block_on(async {
            ...
            loop {
                // The log file doesn't exist until QEMU's first semihosting
                // write; that's "no events yet", not an error.
                let events = match itm::drain_events(&path, 1 << 16).await {
                    Ok(events) => events,
                    Err(HilError::Io(err)) if err.kind() == std::io::ErrorKind::NotFound => {
                        Vec::new()
                    }
    ...
    /// Polls TIM2's counter at a fixed interval for `duration`. `Sample`'s
    /// `readings: Vec<(Duration, bool)>` shape was designed around a pin
    /// toggle; since GPIO isn't observable here (see module doc), each
    /// `bool` instead reports whether the counter changed since the
    /// previous poll — still proves "this backend can observe state
    /// changing over a window," just via TIM2 rather than PA5. The trait
    /// also has no peripheral parameter for `sample` (matches issue #20's
    /// literal `sample(duration_ms)` signature), so TIM2 is the only thing
    /// this backend samples; generalizing is a M6+ concern.
    ...
    fn finish(&mut self) -> RunResult {
        ...
        let status = match &self.skip_reason {
            ...
            None if self.start_time.is_some() => RunStatus::Completed,
...
"#,
    ...
    fn boots_qemu_and_reads_memory_over_gdbstub() {
        ...
        let result: Result<Vec<u8>, HilError> = runtime.block_on(async {
            let mut stub = connect_with_retry("127.0.0.1:12347", &mut child).await?;
