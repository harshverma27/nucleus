...
The remaining crates (`nucleus-itm`, `nucleus-trace`), the React dashboard under `extension/src/dashboard/`, and `xtask/` are still `README.md` placeholders — no source yet. The `nucleus-cli` subcommand `trace` is declared but stubbed (prints a "scheduled for Phase N" notice and exits non-zero) until Phase 5. When asked to "build" or "run" something that doesn't exist, the task is usually to *create* it per the design in `README.md`, not to find existing code.
