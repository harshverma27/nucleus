...
**The entire `nucleus` CLI surface is now live** (`check`/`init`/`build`/`flash`/`lsp`/`trace`) — no stubbed subcommands remain. The React dashboard under `extension/src/dashboard/` (Phase 6) and `xtask/` are still `README.md` placeholders. When asked to "build" or "run" something that doesn't exist, the task is usually to *create* it per the design in `README.md`, not to find existing code.
